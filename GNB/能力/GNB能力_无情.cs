...
public class GNB能力_无情 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (Core.Me.Level == 100)
        {
            ...
            if (Core.Resolve<JobApi_GunBreaker>().Ammo ==2 &&QT.QTGET(QTKey.二弹)&&SpellExtension.CoolDownInGCDs(GNBSpells.血壤, 4)&&(Core.Resolve<MemApiSpellCastSuccess>().LastGcd==GNBSpells.爆发击||Core.Resolve<MemApiSpellCastSuccess>().LastGcd==GNBSpells.命运之环)) return 1;
            if (Core.Resolve<JobApi_GunBreaker>().Ammo ==0 &&QT.QTGET(QTKey.零弹)&&(Core.Resolve<MemApiSpell>().GetLastComboSpellId() == GNBSpells.残暴弹||Core.Resolve<MemApiSpellCastSuccess>().LastGcd==GNBSpells.恶魔切)) return 1;
