...
public class PCTGCD_武器彩绘 : ISlotResolver
{
    public int Check()
    {
        ...
        if (Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.武器彩绘) == PCTSpells.武器彩绘)
        {
            return -4;
        }
    ...
    {
        return Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.武器彩绘).GetSpell();
    }
}