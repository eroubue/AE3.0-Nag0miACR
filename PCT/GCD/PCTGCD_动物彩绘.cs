...
public class PCTGCD_动物彩绘 : ISlotResolver
{
    public int Check()
    {
        ...
        if (Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.动物彩绘) == PCTSpells.动物彩绘)
        {
            return -4;
        }
    ...
    {
        return Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.动物彩绘).GetSpell();
    }
}