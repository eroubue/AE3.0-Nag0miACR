...

{
    public class PCTGCD_CYM : ISlotResolver
    {
        public int Check()
        {
            ...
            if (!Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.冰结之蓝青).IsReady())
            {
                return -3;
            } 
            return 1;
        ...
        {            
            ...
            if (aoeCount >= 4 && Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.冰冻之蓝青).IsReady() &&  QT.QTGET(QTKey.AOE))
             //   && !PCTSettings.Instance.智能aoe目标)
                return Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.冰冻之蓝青).GetSpell();
           // if (Core.Resolve<MemApiSpell>().CheckActionChange(PCTSpells.冰冻之蓝青).IsReady() &&
