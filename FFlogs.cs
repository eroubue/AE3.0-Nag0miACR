...
public class FFlogsAPI
{
    ...
    /// 异步获取FFlogs API的访问令牌
    /// </summary>
    /// <returns>成功时返回访问令牌字符串，失败时抛出异常</returns>
    /// <exception cref="InvalidOperationException">当未配置Client ID或Client Secret时抛出</exception>
  ...
  characterData {{
    ...
    public async Task<(float value, int totalKills, float rankPercent)> GetCurrentJobBestMetricAsync(string characterName, string serverSlug, string region, int encounterID, string currentJob, string metric)
    {
        ...
        try
        {
            ...
            // 检查隐私/无记录
            if (rankings.TryGetProperty("error", out var errorProp) && errorProp.ValueKind == JsonValueKind.String)
            {
                LogHelper.Print($"FFLogs查询失败: {errorProp.GetString()} 原始json: {json.ToString()}");
                return (-1f, -1, -1f);
            }
            ...
            if (totalKills == 0)
            {
                LogHelper.Print($"FFLogs无击杀记录或无权限 原始json: {json.ToString()}");
                return (-1f, -1, -1f);
            }
