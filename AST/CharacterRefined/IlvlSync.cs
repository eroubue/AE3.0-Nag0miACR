...
public static class IlvlSync
{
    public static unsafe (uint?, IlvlSyncType) GetCurrentIlvlSync()
    {
        var icDirector = EventFramework.Instance() != null
            ...
            : null;
        if (icDirector != null)
            if (icDirector->InstanceContentType != InstanceContentType.BeginnerTraining &&
                (icDirector->ContentDirector.Director.ContentFlags & 1) == 0)
            {
                ...
                // duty is synced
                if (((*(byte*)(icd + 7566) & 0x40) == 0 || (UIState.Instance()->PlayerState.IsLevelSynced & 1) != 0) &&
                    *(ushort*)(icd + 1322) > 0)
                    // Service.PluginLog.Debug($"Using duty ilvl sync {*(ushort*)(icd + 1322)}");
                    return (*(ushort*)(icd + 1322), IlvlSyncType.Strict);
            }

        var pcDirector = EventFramework.Instance() != null
            ? EventFramework.Instance()->GetPublicContentDirector()
            : null;
        if (pcDirector != null)
        {
